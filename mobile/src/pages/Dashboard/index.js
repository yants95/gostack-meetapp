...
function Dashboard({ isFocused }) {
    ...
    async function handleRegister(id) {
        try {
            ...
            await showMessage(
                'Atenção!',
                'Ocorreu um erro, contate a organização do evento.'
            );
        }
    }
        ...
        <Background>
            ...
            <DateNavigation>
                <ButtonArrow onPress={handlePrevDay}>
                    <Icon name="chevron-left" size={30} color="#984cc7" />
                </ButtonArrow>
                <Title>{dateFormatted}</Title>
                <ButtonArrow onPress={handleNextDay}>
                    <Icon name="chevron-right" size={30} color="#984cc7" />
                </ButtonArrow>
            </DateNavigation>
            {meetups.length === 0 ? (
                <EmptyView>
                    <EmptyList>
                        <Icon name="event-busy" size={60} color="#984cc7" />
                        <TextEmpty>
                            Não existem eventos programados para hoje.
                        </TextEmpty>
                    </EmptyList>
                </EmptyView>
            ) : (
